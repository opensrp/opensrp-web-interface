...
public class FacilityHelperUtil {
	...
	public void setCHCPTrainingListToSession(HttpSession session,
			List<FacilityTraining> CHCPTrainingList) {
		session.setAttribute("CHCPTrainingList", CHCPTrainingList);
	}

	public void setWorkerTypeListToSession(HttpSession session,
			List<FacilityWorkerType> workerTypeList) {
		session.setAttribute("workerTypeList", workerTypeList);
	}

	public void setFacilityWorkerListToSession(HttpSession session,
			List<FacilityWorker> facilityWorkerList) {
	...
	public void setFacilityWorkerTypeAndTrainingsToSession(HttpSession session) {
		List<FacilityWorkerType> workerTypeList = facilityWorkerTypeService
				.findAll("FacilityWorkerType");
		List<FacilityTraining> CHCPTrainingList = facilityWorkerTrainingService
	...
	public Map<Integer, Integer> getDistinctWorkerCount(
			List<FacilityWorker> facilityWorkerList) {
		List<FacilityWorkerType> workerTypeList = facilityWorkerTypeService
				.findAll("FacilityWorkerType");
	...
	public FacilityWorker convertFacilityWorkerDTO(
			FacilityWorkerDTO facilityWorkerDTO) {
		...
		int facilityWorkerTypeId = Integer.parseInt(facilityWorkerDTO
				.getFacilityWorkerTypeId());
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (position == 0) {
					...
					System.out.println("tags >> " + facilityFromCsv[0] + " >> "
							+ facilityFromCsv[1] + " >> " + facilityFromCsv[3]);
				} else {
					System.out.println(facilityFromCsv[0] + " >> "
							+ facilityFromCsv[1] + " >> " + facilityFromCsv[3]);
					...
					if (facilityFromCsv.length >= 16) {
						if ((facilityFromCsv[13] != null && !facilityFromCsv[13]
								.isEmpty())
								&& (facilityFromCsv[15] != null && !facilityFromCsv[15]
										.isEmpty())) {
							FacilityWorker facilityWorker = new FacilityWorker();
							facilityWorker.setFacility(facility);
							...
							FacilityWorkerType facilityWorkerType = facilityWorkerTypeService
									.findById(1, "id", FacilityWorkerType.class);
							facilityWorker
									.setFacilityWorkerType(facilityWorkerType);
	...
	@Transactional
	public List<String> getAllWorkersNameByKeysWithALlMatchesFromTable(
			String name) {
		Map<String, String> fieldValues = new HashMap<String, String>();
		fieldValues.put("name", name);
		boolean isProvider = false;
		List<FacilityWorker> workerList = repository
				.findAllByKeysWithALlMatches(isProvider, fieldValues,
	...
	@Transactional
	public List<String> getAllWorkersNameByKeysWithALlMatchesFromView(
			String name) {
