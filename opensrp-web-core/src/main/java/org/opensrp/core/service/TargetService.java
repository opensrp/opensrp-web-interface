...
public class TargetService extends CommonService {
	...
	public <T> Integer savePopulationWiseTargetAll(TargetDTO dto) throws Exception {
		...
		try {
			...
			List<TargetCommontDTO> targetList = allTargetUserByUnionWithPopulation(Roles.PK.getId(), LocationTags.UNION_WARD.getId());
			...
			for (TargetCommontDTO target : targetList) {
				...
				for (TargetDetailsDTO targetDetailsDTO : targetDetailsDTOs) {
					...
					targetDetails = targetMapper.targetMapForUnionWiseTarget(targetDetailsDTO, targetDetails,target.getPopulation());
	...
	public List<TargetCommontDTO> allTargetUserByUnionWithPopulation(Integer roleId , Integer locationTagId) {
		...
		try {
			String hql = ""
					+ "SELECT cat.user_id as userId, "
					+ "       cat.location_id as locationId, "
					+ "       wp.population as population, "
					+ "       ub.branch_id as branchId "
					+ "FROM   core.users_catchment_area cat "
					+ "       JOIN core.\"location\" l "
					+ "         ON cat.location_id = l.id "
					+ "       JOIN core.location_tag ltag "
					+ "         ON l.location_tag_id = ltag.id "
					+ "       JOIN core.user_role r "
					+ "         ON r.user_id = cat.user_id "
					+ "       JOIN core.union_wise_population wp "
					+ "         ON l.id = wp.union_id "
					+ "       JOIN core.user_branch ub "
					+ "         ON ub.user_id = cat.user_id "
					+ "WHERE  ltag.id = "+locationTagId+" "
	...
	public JSONObject getUnionWisePopulationSetOfDataTable(Integer draw, int userCount, List<TargetCommontDTO> dtos)
			...
			for (TargetCommontDTO dto : dtos) {
				...
				patient.put(dto.getLocationName());
				patient.put("1200");
				String view = "<div class='col-sm-12 form-group'><a \" href=\"set-individual-target-pk/" + dto.getBranchId() +"/"+ dto.getRoleId() + "/" + dto.getUserId() + ".html?name=" + dto.getFullName()
			        +"&id="+dto.getUsername()+"&location="+dto.getLocationName()+"&population=1200"+"\">Set target</a> </div>";
				patient.put(view);
				array.put(patient);
			}
			response.put("data", array);
	...
	public List<TargetCommontDTO> getTargetInfoForPopulationWise(int roleId, int locationTagId, int month, int year) {
		...
		try {
			String hql = ""
					+ "SELECT td.percentage as percentage, "
					+ "        cast(Coalesce(td.product_id, 0) as integer) as productId, "
					+ "       p.\"name\" as productName, "
					+ "       Coalesce(td.quantity, 0) as quantity "
					+ "FROM   core.target_details td "
					+ "       join core.product p "
					+ "         ON p.id = td.product_id "
					+ "       join core.user_role r "
					+ "         ON td.user_id = r.user_id "
					+ "       join core.users_catchment_area cat "
					+ "         ON cat.user_id = td.user_id "
					+ "       join core.\"location\" l "
					+ "         ON cat.location_id = l.id "
					+ "       join core.location_tag ltag "
					+ "         ON l.location_tag_id = ltag.id "
			...
			Query query = session.createSQLQuery(hql)
					.addScalar("percentage", StandardBasicTypes.STRING)
